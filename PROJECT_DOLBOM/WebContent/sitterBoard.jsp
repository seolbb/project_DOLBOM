						...
						<div class="dtail">
							...
							<div style="margin:8px; text-align: right;">
								<form action="board?" method="post">
									<input type="hidden" name="type" value="searchSitter"> 
							...
							<!-- 요기를 반복처리 -->
							<c:forEach var="vo" items="${sitterList }">
								<a href="board?type=sitterDetail&sb_id=${vo.sb_id }" onclick="return validate()">
								<div class="dtail-main">
									<div>
										<hr class="hrDetail">
									</div>
									...
									<div class="prifile-container">
										<div class="profile-left">
											<img class="profile-Image" src="${pageContext.request.contextPath }/images/${vo.ori_name }" alt="">
										</div>
							...
							<td colspan="3">
								<ol class="paging">
									<c:choose>
										...
										<c:otherwise>
											<li><a
												href="board?type=sitterBoard&cPage=${pvo.beginPage - 1 }">이전으로</a>
											</li>
										</c:otherwise>
									</c:choose>
									...
									<c:forEach var="pageNo" begin="${pvo.beginPage }"
										...
										<c:choose>
											...
											<c:otherwise>
												<li><a
													href="board?type=sitterBoard&cPage=${pageNo }">${pageNo }</a></li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									...
									<c:if test="${pvo.endPage < pvo.totalPage }">
										<li><a
											href="board?type=sitterBoard&cPage=${pvo.endPage + 1 }">다음으로</a></li>
									</c:if>
									<c:if test="${pvo.endPage >= pvo.totalPage }">
									...
									<li><input type="button" id="btn-Cr"
										class="btn btn-info" value="시터등록" onclick="sendData()">
									</li>
									</c:if>
								</ol>
