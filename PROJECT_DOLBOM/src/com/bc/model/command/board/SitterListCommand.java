...
public class SitterListCommand implements Command {
	...
	public String exec(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 전체 게시물의 수량 구하기	
		p.setTotalRecord(SitterboardDAO.getTotalCount()); 
		p.setTotalPage();
	...
	*/
		...
		//  DB 연결하고 데이터 가져오기
		List<SitterboardVO> list = SitterboardDAO.getSitterList(p.getBegin(),p.getEnd());
		...
		request.setAttribute("sitterList", list);
		request.setAttribute("pvo", p);
		
		return "sitterBoard.jsp";
